"""
This module implements a multithreaded TCP echo server that listens for incoming connections
on a given port.
The server listens on all interfaces and IP addresses on the machine.
The server echoes any messages sent to it.

The server is capable of handling multiple clients concurrently by creating a new thread for each
incoming connection.
"""
import socket
import logging
